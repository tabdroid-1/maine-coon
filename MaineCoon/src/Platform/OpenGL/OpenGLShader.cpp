...
namespace TabbyEngine{
    static GLenum ShaderTypeFromString(const std::string& type){
        ...
        TABBY_CORE_ASSERT(false, "unknown shader type");
        return 0;
    }
    ...
    std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source){
        ...
        while(pos != std::string::npos){
            ...
            TABBY_CORE_ASSERT(eol != std::string::npos, "Syntax error");
    ...
    std::string OpenGLShader::ReadFile(const std::string &filepath) {
        ...
        if(in){
            ...
            TABBY_CORE_ERROR("Could not open file {0}", filepath);
            TABBY_CORE_INFO("Current working dir: {0}",(void*)std::filesystem::current_path().c_str());
        }
        return result;
    }
    ...
    void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string> &shaderSource) {
        ...
        TABBY_CORE_ASSERT(shaderSource.size() <= 2, "Only 3 shaders are supported");
        ...
        for(auto& kv_pair : shaderSource){
           ...
           if(isCompiled == GL_FALSE)
           {
               ...
               TABBY_CORE_ERROR("Shader compilation error!");
               TABBY_CORE_ERROR("{0}", infoLog.data());
               TABBY_CORE_ASSERT(false, "Shader compilation issue");
        ...
        if (isLinked != 1)
        {
            ...
            TABBY_CORE_ERROR("Linking shader error!");
            TABBY_CORE_ERROR("{0}", infoLog.data());
            TABBY_CORE_ASSERT(false, "Shader linking issue");
            return;
        }
        for (auto id: glShaderIDs)
