...
namespace TabbyEngine{
    ...
    :m_Path(path){
        ...
        {
            TABBY_PROFILE_SCOPE("SOIL Load - OpenGLTexture2D::OpenGLTexture2D(const char* path)");
        ...
        //TABBY_CORE_ASSERT(textureref == 1, "Failed to load image!");
        if (m_RendererID == 0){
            TABBY_CORE_ERROR("Couldn't find requested texture file: {0}", m_Path);
            namespace fs = std::filesystem;
            TABBY_CORE_INFO("System is searching here: {0}", (void*)fs::current_path().c_str());
        }
        m_Width = width;
        m_Height = height;
    ...
    void OpenGLTexture2D::SetData(void* data, uint32_t size, uint32_t slot) const{
        ...
        TABBY_CORE_ASSERT(size == m_Width* m_Height * bpp, "Data must be entire texture");
