...
namespace TabbyEngine{
    static GLenum ShaderDataTypeToGLBase(ShaderDataType type){
        ...
        TABBY_CORE_ASSERT(false, "Unknown ShaderDataType");
        return 0;
    }
    ...
    void OpenGLVertexArray::AddVertexBuffer(const Ref<VertexBuffer> &vertexBuffer) {
        TABBY_PROFILE_FUNCTION();
        TABBY_CORE_ASSERT(vertexBuffer->GetLayout().GetElements().size(),"Vertex Buffer has no layout");
        glBindVertexArray(m_RendererID);
        vertexBuffer->Bind();

        const auto& layout = vertexBuffer->GetLayout();
        TABBY_CORE_INFO("___VERTEX ATTRIB____");
        for(const auto& element: layout){
            switch (element.Type) {
                ...
                case ShaderDataType::Float4:
                {
                    ...
                    TABBY_CORE_INFO("index {}: comp count:{}, Type:{}, layout:{} ,offset {}",
                ...
                default: TABBY_CORE_ASSERT(false,"Unknown ShaderType");
