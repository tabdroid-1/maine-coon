...
namespace TabbyEngine{
    ...
    void OpenGLUtils::PrintShaderLog(GLuint shader){
        ...
        if(len> 0){
            ...
            TABBY_CORE_INFO("Shader log info: {0}",log);
            free(log);
        }
    }
    bool OpenGLUtils::CheckOpenGLError() {
        ...
        while(glErr != GL_NO_ERROR){
            TABBY_CORE_ERROR("OpenGL error: {0}", glErr);
            found_Err = true;
            glErr = glGetError();
        }
        return found_Err;
    ...
    void OpenGLUtils::printProgramLog(int prog) {
        ...
        if (len > 0){
            ...
            TABBY_CORE_INFO("Program Log: {0}",log);
            free(log);
        }
    }
    void OpenGLUtils::displayComputerShaderLimits(){
        ...
        TABBY_CORE_INFO("maximum number of workgroups is:{0} {1} {2}",
                     work_grp_cnt[0], work_grp_cnt[1], work_grp_cnt[2]
                     );

        TABBY_CORE_INFO("maximum size of workgroups is:{0} {1} {2}",
                     work_grp_siz[0], work_grp_siz[1], work_grp_siz[2]
        );

        TABBY_CORE_INFO("max local work group invocations:{0}",work_grp_inv);
    }
    std::string OpenGLUtils::readShaderFile(const char *filepath) {
        ...
        string line = "";
        if(!fileStream){
            TABBY_CORE_WARN("Couldnt find file: {0}",filepath);
            return "";
        }else{
